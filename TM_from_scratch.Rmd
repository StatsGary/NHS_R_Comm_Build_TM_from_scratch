...
```{r training_model, echo=FALSE, include=TRUE, results='hide'}
...
predictions <- predictions %>% 
  ...
  dplyr::select(everything(), -c(.pred_class, .pred_negative)) %>% 
  dplyr::mutate(log_reg_class_custom = ifelse(xgboost_model_prob >0.7,"sick","negative")) %>% 
  dplyr::select(-.pred_sick)
...
```{r extract_eval_data}
tibble(
  Precision=cm_lr$record_level_cm$Precision,
  Recall=cm_lr$record_level_cm$Recall
)
